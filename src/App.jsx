...
// ============================================================================
// --- SECTION 3: LANDING SCREEN (REMOTE UNLOCK FIX) ---
// ============================================================================

const LandingScreen = ({ onEnter, securityEnabled }) => {
  const [pinMode, setPinMode] = useState(false); // Editor PIN
  const [visitorPinMode, setVisitorPinMode] = useState(false); // Visitor Manual PIN
  
  const [isRequesting, setIsRequesting] = useState(false); // Waiting Screen
  ...
  // LISTENER: Tunggu Editor Approve
  useEffect(() => {
...
// ============================================================================
// --- SECTION 4: STUDIO & CAROUSEL (SAME AS BEFORE) ---
// ============================================================================
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center animate-fade-in">
            <div className="bg-[#1e293b] w-96 p-6 rounded-2xl border border-white/10 shadow-2xl">
                ...
                <div className="bg-[#0f172a] p-4 rounded-xl border border-white/5 mb-4"><div className="flex justify-between items-center"><div><p className="text-sm font-bold text-white mb-1">Visitor 2FA Security</p><p className="text-[10px] text-slate-400">Require approval to enter</p></div><button onClick={() => setSecurityEnabled(!securityEnabled)} className={`w-12 h-6 rounded-full transition-colors relative ${securityEnabled ? 'bg-indigo-500' : 'bg-slate-600'}`}><div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${securityEnabled ? 'translate-x-6' : 'translate-x-0'}`} /></button></div></div>
