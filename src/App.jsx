...
// ============================================================================
// --- SECTION 2: FLIP VIEW (CALM BACKGROUND + MOTIF) ---
// ============================================================================
const FlipView = ({ pages, coverUrl, backCoverUrl, onClose, title }) => {
  ...
  useEffect(() => {
      const handleResize = () => {
          if (window.innerWidth < 640) { setDimensions({ width: window.innerWidth - 20, height: (window.innerWidth - 20) * 1.4 }); } 
  ...
  return (
    // UPDATED BACKGROUND: Calm Dark + Subtle Pattern
    ...
    <div className="fixed inset-0 bg-[#f0f2f5] flex flex-col items-center justify-center font-sans select-none p-4">
      ...
      <div className="z-10 relative flex flex-col items-center w-full max-w-sm md:max-w-md">
        {!successMode && !isRequesting && !pinMode && requestStatus !== "denied" && (
             <div className="mb-8 md:mb-10 animate-fade-in flex flex-col items-center"><div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-pink-500 to-violet-600 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center shadow-2xl shadow-pink-500/20 mb-4 md:mb-6"><Camera className="text-white" size={28} md:size={32} strokeWidth={2.5} /></div><div className="text-center"><h1 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tight mb-2">Lumière OS</h1><p className="text-[10px] font-bold tracking-[0.3em] text-slate-400 uppercase">Select Your Journey</p></div></div>
        )}
        {!pinMode && !successMode && !isRequesting && requestStatus !== "denied" && (
          <div className="flex flex-row gap-4 md:gap-6 animate-slide-up">
            <button onClick={() => setPinMode(true)} className="group w-32 h-40 md:w-36 md:h-44 bg-white/40 backdrop-blur-xl border border-white/60 rounded-[1.5rem] md:rounded-[2rem] flex flex-col items-center justify-center gap-3 md:gap-4 hover:bg-white/60 hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl shadow-slate-200/50"><div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-gradient-to-br from-pink-500 to-rose-500 flex items-center justify-center shadow-lg group-hover:shadow-pink-500/30 transition-all"><Lock className="text-white" size={18} md:size={20} /></div><span className="text-[10px] md:text-[11px] font-bold text-slate-600 tracking-wider uppercase">Editor</span></button>
            <button onClick={handleVisitorClick} onMouseDown={securityEnabled ? startPress : null} onMouseUp={securityEnabled ? cancelPress : null} onMouseLeave={securityEnabled ? cancelPress : null} onTouchStart={securityEnabled ? startPress : null} onTouchEnd={securityEnabled ? cancelPress : null} className={`group relative w-32 h-40 md:w-36 md:h-44 backdrop-blur-xl border border-white/60 rounded-[1.5rem] md:rounded-[2rem] flex flex-col items-center justify-center gap-3 md:gap-4 hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl shadow-slate-200/50 overflow-hidden ${securityEnabled ? 'bg-white/40 hover:bg-white/60' : 'bg-green-500/10 border-green-500/20 hover:bg-green-500/20'}`}>{securityEnabled && <div className="absolute bottom-0 left-0 right-0 bg-violet-500/10 transition-all duration-75" style={{ height: `${progress}%` }} />}<div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center shadow-lg transition-all relative z-10 ${securityEnabled ? 'bg-gradient-to-br from-violet-500 to-purple-600' : 'bg-green-500'}`}>{securityEnabled ? <Eye className="text-white" size={18} md:size={20} /> : <Unlock className="text-white" size={18} md:size={20} />}</div><div className="flex flex-col items-center z-10"><span className="text-[10px] md:text-[11px] font-bold text-slate-600 tracking-wider uppercase">Visitor</span><span className={`text-[7px] md:text-[8px] uppercase mt-1 font-bold ${securityEnabled ? 'text-slate-400' : 'text-green-600'}`}>{securityEnabled ? (progress > 0 ? "Hold..." : "Hold to Request") : "Unlocked"}</span></div></button>
          </div>
        )}
        {isRequesting && requestStatus === "waiting" && (<div className="flex flex-col items-center animate-slide-up bg-white/40 backdrop-blur-xl border border-white/60 p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl shadow-slate-200/50 w-full mx-4 text-center"><div className="w-14 h-14 md:w-16 md:h-16 bg-white rounded-full flex items-center justify-center shadow-lg mb-4 md:mb-6 relative"><div className="absolute inset-0 rounded-full border-4 border-violet-100 border-t-violet-500 animate-spin" /><Smartphone size={20} md:size={24} className="text-slate-400"/></div><h3 className="text-slate-800 text-lg md:text-xl font-black mb-2">Approval Required</h3><p className="text-xs md:text-sm text-slate-500 leading-relaxed mb-6 md:mb-8">Notification sent to Owner.<br/>Waiting for remote unlock...</p><button onClick={() => {setIsRequesting(false); setRequestStatus("idle");}} className="text-xs text-slate-400 font-bold tracking-widest uppercase hover:text-slate-600 py-2">Cancel</button></div>)}
        {requestStatus === "denied" && (<div className="flex flex-col items-center animate-bounce bg-red-50/90 backdrop-blur-xl border border-red-200 p-8 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl shadow-red-500/20 w-full mx-4 text-center"><div className="w-16 h-16 md:w-20 md:h-20 bg-red-500 rounded-full flex items-center justify-center shadow-lg mb-4 md:mb-6 text-white"><AlertCircle size={32} md:size={40} strokeWidth={3}/></div><h3 className="text-red-900 text-xl md:text-2xl font-black mb-2 uppercase tracking-wide">Access Denied</h3><p className="text-xs md:text-sm text-red-700 font-medium leading-relaxed">Your request was declined.</p></div>)}
        {pinMode && !successMode && (<div className="flex flex-col items-center animate-slide-up bg-white/40 backdrop-blur-xl border border-white/60 p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl shadow-slate-200/50 w-full mx-4"><h3 className="text-slate-800 text-lg md:text-xl font-bold mb-1">Editor Access</h3><p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-6 md:mb-8">Enter Passcode</p><div className="flex gap-2 md:gap-3 mb-8 md:mb-10">{[0,1,2,3,4,5].map((_, i) => (<div key={i} className={`w-3 h-3 md:w-3 md:h-3 rounded-full transition-all duration-300 ${pin.length > i ? 'bg-gradient-to-br from-pink-500 to-rose-600 scale-125' : 'bg-slate-300'}`} />))}</div><input autoFocus type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6} value={pin} onChange={(e) => { const val = e.target.value; if (!/^\d*$/.test(val)) return; setPin(val); if(val.length === 6) handleEditorLogin(val); }} className="opacity-0 absolute inset-0 cursor-pointer h-full w-full z-50" />{error && <p className="text-rose-500 text-xs font-bold uppercase animate-pulse mb-4">Incorrect PIN</p>}<button onClick={() => {setPinMode(false); setPin("");}} className="mt-2 px-6 py-2 rounded-full border border-slate-300 text-xs font-bold text-slate-400 hover:text-slate-800 hover:bg-white uppercase tracking-widest transition-all z-50">Cancel</button></div>)}
        {successMode && (<div className="flex flex-col items-center animate-scale-up text-center bg-white/40 backdrop-blur-xl border border-white/60 p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl"><div className="w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br from-pink-500 to-violet-600 rounded-[2rem] flex items-center justify-center shadow-2xl shadow-pink-500/30 mb-6 md:mb-8"><Camera className="text-white" size={36} md:size={40} strokeWidth={2.5} /></div><h1 className="text-2xl md:text-3xl font-black text-slate-800 mb-2">Welcome, Editor</h1><p className="text-xs md:text-sm text-slate-500 font-medium mb-10 md:mb-12">You are now inside the Lumière System.</p><div className="flex flex-col gap-3 md:gap-4 w-56 md:w-64"><button onClick={() => onEnter('editor')} className="px-8 py-3 md:py-4 bg-slate-900 text-white rounded-full font-bold text-[10px] md:text-xs tracking-[0.2em] hover:bg-black hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-2 uppercase">Enter System <ArrowRight size={14}/></button><button onClick={handleLogout} className="px-8 py-3 md:py-4 bg-white text-slate-900 border border-slate-200 rounded-full font-bold text-[10px] md:text-xs tracking-[0.2em] hover:bg-slate-50 transition-all shadow-sm uppercase">Log Out</button></div></div>)}
      </div>
    </div>
  );
};
...
// ============================================================================
// --- SECTION 4: CAROUSEL (SWIPE + SCROLL ADDED) ---
// ============================================================================
const Carousel3D = ({ books, onSelect }) => {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => { if (e.deltaY > 0) nextSlide(); else prevSlide(); }; // Scroll support
    
    window.addEventListener('keydown', handleKeyDown); 
    // Add wheel listener to window/container (not passive to allow control)
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [activeIndex, books]);

  // TOUCH SWIPE LOGIC FOR LIBRARY
  ...
  return (
    <div 
        className="w-full h-full flex flex-col items-center justify-center relative overflow-hidden touch-pan-y" 
        onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} 
        onWheel={(e) => { if(Math.abs(e.deltaY) > 20) e.deltaY > 0 ? nextSlide() : prevSlide() }}
    >
...
// ... (REST OF THE CODE: CreateStudio, Sidebar, etc. remains the same as previous corrected version) ...
// (To save space, assume CreateStudio, Sidebar, SettingsModal, RequestsModal, LumierePro are here)
// ...
...
export default function LumierePro() {const [books, setBooks] = useState([{ id: '1', title: 'Graduation Day', coverUrl: 'https://picsum.photos/400/520?random=1', pageCount: 12 }, { id: '2', title: 'Japan Trip', coverUrl: 'https://picsum.photos/400/520?random=2', pageCount: 8 }, { id: '3', title: 'Wedding', coverUrl: 'https://picsum.photos/400/520?random=3', pageCount: 24 }]);const [view, setView] = useState('landing'); const [userRole, setUserRole] = useState('visitor'); const [currentBook, setCurrentBook] = useState(null);const [securityEnabled, setSecurityEnabled] = useState(true); const [isSettingsOpen, setIsSettingsOpen] = useState(false);const [isRequestsOpen, setIsRequestsOpen] = useState(false);const [visitorWaiting, setVisitorWaiting] = useState(false);useEffect(() => {if (userRole === 'editor') {const statusRef = ref(db, 'access/settings/securityEnabled');const unsubSecurity = onValue(statusRef, (snapshot) => {const val = snapshot.val();setSecurityEnabled(val !== false);});const statusRef2 = ref(db, 'access/status');const unsubStatus = onValue(statusRef2, (snapshot) => {const val = snapshot.val();const isPending = val === "pending";setVisitorWaiting(isPending);if (isPending) setIsRequestsOpen(true);});return () => { unsubSecurity(); unsubStatus(); }} else {const statusRef = ref(db, 'access/settings/securityEnabled');const unsubSecurity = onValue(statusRef, (snapshot) => {const val = snapshot.val();setSecurityEnabled(val !== false);});return () => unsubSecurity();}}, [userRole]);const handleToggleSecurity = () => {const newValue = !securityEnabled;set(ref(db, 'access/settings/securityEnabled'), newValue);};const handleApprove = () => { set(ref(db, 'access/status'), "approved"); setVisitorWaiting(false); setIsRequestsOpen(false); };const handleDeny = () => { set(ref(db, 'access/status'), "denied"); setVisitorWaiting(false); setIsRequestsOpen(false); };const [studioTitle, setStudioTitle] = useState(''); const [studioAudio, setStudioAudio] = useState(''); const [studioCover, setStudioCover] = useState(null); const [studioBackCover, setStudioBackCover] = useState(null); const [studioPages, setStudioPages] = useState([]); const [isSaving, setIsSaving] = useState(false); const [statusMsg, setStatusMsg] = useState("");const handleLogin = (role) => { setUserRole(role); setView('dashboard'); };const handleOpenStudio = (book) => { if (book) { setStudioTitle(book.title); setStudioCover(book.coverUrl); setStudioBackCover(book.backCoverUrl); setStudioPages(book.pages || []); } else { setStudioTitle(''); setStudioCover(null); setStudioBackCover(null); setStudioPages([]); } setCurrentBook(book); setView('studio'); };const handleUploadCover = async (event) => { const file = event.target.files[0]; if(!file) return; const url = await compressImage(file); setStudioCover(url); };const handleUploadBackCover = async (event) => { const file = event.target.files[0]; if(!file) return; const url = await compressImage(file); setStudioBackCover(url); };const handleUploadPages = async (event) => { const files = Array.from(event.target.files); setStatusMsg('Compressing...'); setIsSaving(true); const uploaded = []; for (const file of files) { const url = await compressImage(file); uploaded.push({ url, caption: file.name }); } setStudioPages(p => [...p, ...uploaded]); setStatusMsg('Ready'); setIsSaving(false); };const handlePublish = () => { setIsSaving(true); setTimeout(() => { const newBook = { id: Date.now().toString(), title: studioTitle || 'Untitled', coverUrl: studioCover || 'https://picsum.photos/400/520?random=9', backCoverUrl: studioBackCover, audioUrl: studioAudio, pageCount: studioPages.length, pages: studioPages }; setBooks(prev => [newBook, ...prev]); setIsSaving(false); setView('dashboard'); }, 1000); };return (<div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans overflow-hidden h-screen w-screen selection:bg-indigo-500/30"><SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} securityEnabled={securityEnabled} toggleSecurity={handleToggleSecurity} /><RequestsModal isOpen={isRequestsOpen} onClose={() => setIsRequestsOpen(false)} visitorRequest={visitorWaiting} onApprove={handleApprove} onDeny={handleDeny} />{view === 'landing' && <LandingScreen onEnter={handleLogin} securityEnabled={securityEnabled} />}{view === 'reader' && currentBook && ( <FlipView pages={currentBook.pages || []} coverUrl={currentBook.coverUrl} backCoverUrl={currentBook.backCoverUrl} title={currentBook.title} onClose={() => setView('dashboard')} /> )}{view === 'studio' && ( <CreateStudio onClose={() => setView('dashboard')} title={studioTitle} setTitle={setStudioTitle} audio={studioAudio} setAudio={setStudioAudio} cover={studioCover} setCover={setStudioCover} backCover={studioBackCover} setBackCover={setStudioBackCover} pages={studioPages} setPages={setStudioPages} onUploadCover={handleUploadCover} onUploadBackCover={handleUploadBackCover} onUploadPages={handleUploadPages} onPublish={handlePublish} isSaving={isSaving} statusMsg={statusMsg} /> )}{view === 'dashboard' && (<div className="flex flex-col md:flex-row h-screen w-full overflow-hidden"><Sidebar userRole={userRole} onLogout={() => setView('landing')} onStudio={() => handleOpenStudio(null)} openSettings={() => setIsSettingsOpen(true)} openRequests={() => setIsRequestsOpen(true)} visitorRequest={visitorWaiting} /><div className="flex-1 h-full overflow-hidden bg-[#0f172a] relative flex flex-col min-h-0"><div className="flex-1 relative min-h-0"><div className="absolute inset-0 bg-gradient-to-t from-indigo-900/10 via-transparent to-transparent pointer-events-none z-0"/>{userRole === 'visitor' || userRole === 'editor' ? ( <Carousel3D books={books} onSelect={(book) => { setCurrentBook(book); setView('reader'); }} /> ) : null}</div></div></div>)}</div>);}